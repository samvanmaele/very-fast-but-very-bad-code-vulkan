...
void FrameManager::init(VkDevice &device, SDL_Window* window, VkSurfaceKHR &surface, QueueFamilyIndices &indices, SwapChainSupportDetails &swapChainSupport)
{
...
void FrameManager::createSwapChain(VkDevice &device, SDL_Window* window, VkSurfaceKHR &surface, QueueFamilyIndices &indices, SwapChainSupportDetails &swapChainSupport)
{
    ...
    uint32_t imageCount = std::max(swapChainSupport.capabilities.minImageCount, (uint)MAX_FRAMES_IN_FLIGHT) + 1;
